...
typename AVLtree<T>::Node* AVLtree<T>::remove(Node* node, const T& value)
{
    ...
    if (node->data_ > value)
    {
        node->leftChild_ = remove(node->leftChild_, value);
        //判断是否失衡
        if (height(node->rightChild_) - height(node->leftChild_) > 1)
        {
            //右子树的右孩子高
    ...
    {
        node->rightChild_ = remove(node->rightChild_, value);
        //判断是否失衡
        if (height(node->leftChild_) - height(node->rightChild_) > 1)
        {
            //左子树的左孩子高
    ...
    else
    {
        ...
        {
            if (height(node->leftChild_) >= height(node->rightChild_))
            {
                //寻找前驱节点
            ...
            {
                Node* post = node->rightChild_;
                //寻找后继节点
                while (post->leftChild_ != nullptr)
                {
                    post = post->leftChild_;
                }
                node->data_ = post->data_;
                node->rightChild_ = remove(node->rightChild_, post->data_);
            }
            //有一个孩子
        }
    ...
    //高度更新
    node->height_ = std::ranges::max(height(node->leftChild_), height(node->rightChild_)) + 1;
    return node;
}
