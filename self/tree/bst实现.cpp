...
//最近公共祖先节点实现
/*思路：首先判断root节点是否为空 然后从root开始遍历 当某个节点第一次开始满足大于left且小于right时 此节点为最近公共祖先节点
 */
template<typename T, typename Comp>
BSTree<T, Comp>::Node *BSTree<T, Comp>::getLCA(int val1, int val2) {
    if (root_ == nullptr) {
        throw "root is null";
    }
    int left = (val1 < val2) ? val1 : val2;
    int right = (val1 < val2) ? val2 : val1;
    Node *LCA = root_;
    while (LCA != nullptr) {
        if (comp_(left, LCA->data_) && comp_(LCA->data_, right)) {
            break;
        }
        if (comp_(left, LCA->data_) && comp_(right, LCA->data_)) {
            LCA = LCA->leftChild;
        } else {
            LCA = LCA->rightChild;
        }
